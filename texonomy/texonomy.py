...
class Entity:
    ...
    def __str__(self) -> None:
        return f"{self.name} ({'Weak ' if self.weak else ''}Entity)\n" \
    ...
    def to_latex(self) -> str:
        code = f"{{{'weak' if self.weak else ''}" \
               f"entity={{{id(self)}}}{{{self.name}}}{{%"
        for idx, attribute in enumerate(self.attributes):
            code += f"\n{INDENTATION[-1]}"
            code += f"\{'dashuline' if self.weak else 'underline'}" \
                    fr"{{{attribute}}}\\" if idx == self.primary \
                                                else fr"{attribute}\\"
        return code + f"\n{INDENTATION[-2]}}}}};"
...
def determine_line_type(cardinality: Tuple[Tuple[int, int], Tuple[int, int]],
                            direction: int) -> str:
...
class ERDiagram:
    ...
    def __init__(self, entity: Entity) -> None:
        ...
        self.code = f"{INDENTATION[-2]}\pic {entity.to_latex()}\n"

    def add_relationship(self, rel: Relationship,
                         defining: bool=False) -> None:
        ...
        self.code += f"{INDENTATION[-2]}\pic[{rel.direction.value}=3em " \
                     f"of {id(rel.anchor)}] {{{'def' if defining else ''}" \
                     f"relationship={{{id(rel)}}}{{{rel.label}}}}};\n"

        # render other entity
        self.code += f"{INDENTATION[-2]}\pic[{rel.direction.value}=3em " \
                     f"of {id(rel)}] {rel.new_entity.to_latex()}\n"
        ...
        self.code += f"{INDENTATION[-2]}\draw[{line_type_one}] " \
                     f"({id(rel)}.{line_anchors[0]}) -- " \
                     f"({id(rel.anchor)}.{line_anchors[1]});\n"

        # render line between new entity and relationship
        self.code += f"{INDENTATION[-2]}\draw[{line_type_two}] " \
                     f"({id(rel)}.{line_anchors[1]}) -- " \
                     f"({id(rel.new_entity)}.{line_anchors[0]});\n"

        if rel.attributes:
            direction = Direction.RIGHT if rel.direction == Direction.ABOVE \
                        or rel.direction == Direction.BELOW \
            ...
            self.code += f"{INDENTATION[-2]}\pic[{direction.value}=2em of " \
                         f"{id(rel)}] {rel.attributes_to_latex()}\n"
            # latex node id is "a" + rel id
            self.code += f"{INDENTATION[-2]}\draw[dashed line] ({id(rel)}." \
                         f"{line_anchors[1]}) -- (a{id(rel)}." \
                         f"{line_anchors[0]});\n"

    def add_specialization(self, superclass: Entity,
                           subclass: Entity) -> None:
        if superclass not in self.entities:
            raise ValueError("Anchor does not exist in the diagram.")
        self.entities.append(subclass)

        # add other entity
        self.code += f"{INDENTATION[-2]}\pic[below=3em of {id(superclass)}]" \
                     f" {subclass.to_latex()}\n"
        self.code += f"{INDENTATION[-2]}\draw[specialization] " \
                     f"({id(subclass)}.north) -- ({id(superclass)}.south);\n"

    def to_latex(self, template_file: str) -> str:
        prelude = ""
        # I tried including the file as a part of the package and it was the
        # most frustrating thing I've ever done. So for now, the template file
        # will have to be locally provided...
        with open(template_file) as template:
            prelude += template.read()

        return prelude + f"{INDENTATION[0]}\\begin{{document}}\n" \
